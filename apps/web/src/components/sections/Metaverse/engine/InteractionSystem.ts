...
export default class InteractionSystem {
  ...
  public update() {
    ...
    this.activeZone = this.zones.find((zone) =>
      this.isPlayerInZone(bounds, zone),
    );
    ...
    this.graphics
      ...
      .stroke({ color: 0xffff00, width: 1, alpha: 0.8 }).visible = true;
  ...
  private layoutPrompt(promptText: string) {
    ...
    const afterX = xMatch
      ? text.slice(xMatch.index + xMatch[0].length).trim()
      : "to interact";
    ...
    const contentHeight = Math.max(
      this.promptPrefix.height,
      this.promptSuffix.height,
      keyHeight,
    );
    const contentWidth =
      this.promptPrefix.width +
      contentGap +
      keyWidth +
      contentGap +
      this.promptSuffix.width;
    ...
    const keyCenterX =
      this.promptPrefix.x + this.promptPrefix.width + contentGap + keyWidth / 2;
    const keyCenterY = contentY;
    this.keyBackground
      .clear()
      .roundRect(
        keyCenterX - keyWidth / 2,
        keyCenterY - keyHeight / 2,
        keyWidth,
        keyHeight,
        4,
      )
    ...
    this.promptSuffix.position.set(
      keyCenterX + keyWidth / 2 + contentGap,
      contentY,
    );
  }
