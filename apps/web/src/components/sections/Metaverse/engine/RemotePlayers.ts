...
export default class RemotePlayers {
  ...
  async updateRemotePlayers(players: Players) {
    ...
    for (const id of Object.keys(players)) {
      ...
      if (!sprite)
        sprite = await this.getOrCreateRemotePlayer(id, remotePlayer);

      const remoteContainer =
        this.socketIdToRemotePlayersMap.get(id)?.container;
      if (remoteContainer) {
  ...
  private getOrCreateRemotePlayer(
    id: string,
    player: PlayerMoveData,
  ): Promise<AnimatedSprite> {
    const existingSprite =
      this.socketIdToRemotePlayersMap.get(id)?.animatedSprite;
    if (existingSprite) return Promise.resolve(existingSprite);

    const inFlightCreation =
      this.socketIdToRemotePlayersMap.get(id)?.animatedSpriteCreationPromise;
  ...
  private async createRemotePlayer(
    id: string,
    player: PlayerMoveData,
  ): Promise<AnimatedSprite> {
  ...
  private async getSpriteSheet(avatar: AvatarId): Promise<Spritesheet> {
    ...
    const nextSheetPromise = Assets.load(selectedAvatar.texture).then(
