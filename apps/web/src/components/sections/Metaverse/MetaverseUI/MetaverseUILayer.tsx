...
export default function MetaverseUILayer({
  socketClient,
  handleKeyPress,
}: Props) {
  ...
  const [fullScreenRemoteUser, setFullScreenRemoteUser] = useState<
    string | null
  >(null); //remote user
  const [remoteVideos, setRemoteVideos] = useState<IRemoteVideos>({});
  const [remotePeerUsernames, setRemotePeerUsernames] =
    useState<IRemotePeerUsernames>({});
