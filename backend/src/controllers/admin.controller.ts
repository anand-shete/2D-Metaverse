...
export const createElement = async (req: FastifyRequest, res: FastifyReply) => {
  try {
    ...
    if (!parsedData.success) return res.status(400).send({ message: "Validation Error" });
    ...
    return res.status(201).send({ message: "Element Created sucessfully", elementId: element._id });
  } catch (error) {
    console.log(error);
...
export const updateElementById = async (req: FastifyRequest, res: FastifyReply) => {
  try {
    ...
    if (!elementId) return res.status(404).send({ message: "Element ID not found" });

    await Element.updateOne({ _id: elementId }, { imageUrl: parsedData.data.imageUrl });
  } catch (error) {
    console.log(error);
...
export const createAvatar = async (req: FastifyRequest, res: FastifyReply) => {
  try {
    ...
    return res.status(201).send({ message: "Avatar Created sucessfully", avatarId: avatar._id });
  } catch (error) {
    console.log(error);
...
export const createMap = async (req: FastifyRequest, res: FastifyReply) => {
  try {
    ...
    return res.status(200).send({ message: "Map created successfully", mapId: map._id });
  } catch (error: any) {
